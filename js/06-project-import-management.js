...
function extractProjectBatchPayload(rows){
  ...
  if(!headers.length || !dataRows.length){
    return { recognized:false, reason:'헤더 또는 데이터 행이 없어' };
  }
  ...
  return {
    recognized,
    reason: recognized ? '' : '인식 가능한 헤더가 없어',
    payload: result
  };
}
async function readProjectBatchRowsFromFile(file){
  ...
  throw new Error('지원하지 않는 파일 형식이야. csv / xlsx / xls만 가능해.');
}
  ...
  for(const file of list){
    ...
    try{
      ...
      if(!parsed.recognized){
        failCount++;
        logs.push(`실패 · ${file.name} · ${parsed.reason || '인식 가능한 헤더가 없어'}`);
        continue;
      }
      if(!project){
